...
//ofstream lgot("log_acquaintance.txt");	//file to store Infect/Immune nodes
ofstream fo ("acquaintance.csv");	//file to store node num data
//ofstream fotemp("tempdata_acquaintance.csv");//temp data in case program stop
...
int main(){
	...
	//	ranchoose = RANDC; //for test sample
		...
		for(int imtime=0;imtime<STOPTIME;immucontrol){	
			IFt.clear();
			#ifdef DEBUG 
			...
			int ranneigh;
			if(!neighbor.empty())
				ranneigh=neighbor[rand()/(RAND_MAX+1.0)*neighbor.size()];
			else
				ranneigh=ranchoose;
			availimremove(ranneigh);	//remove from available im list
			ifectvecremove(ranneigh);	//remove from infected list
			IM.push_back(ranneigh);	//add to im list
			#ifdef DEBUG 
			cout<<"Select node No."<<ranchoose+1<<endl;
			cout<<"Node No."<<ranchoose+1<<" has "<<neighbor.size()<<" neighbor nodes"<<endl;
			cout<<"Randomly immunize its neighbor node No."<<ranneigh+1<<endl;
			#endif
			//lgot<<"M:"<<ranneigh+1<<endl;
